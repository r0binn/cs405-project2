...
class MeshDrawer {
  ...
  constructor() {
    ...
    this.ambientPoint = gl.getUniformLocation(this.prog, "ambientI");
    this.enableLightParam = gl.getUniformLocation(this.prog, "enableLighting");
    this.normalBuffer = gl.createBuffer();
    this.normalPoint = gl.getAttribLocation(this.prog, "normal");
  }
  ...
  draw(trans) {
    ...
    gl.bindBuffer(gl.ARRAY_BUFFER, this.normalBuffer);
    gl.enableVertexAttribArray(this.normalPoint);
    gl.vertexAttribPointer(this.normalPoint, 3, gl.FLOAT, false, 0, 0);

    gl.uniform3fv(this.lightPos, [lightX, lightY, lightZ]);
  ...
  enableLighting(show) {
     ...
     */
    gl.useProgram(this.prog);
    gl.uniform1i(this.enableLightParam, show);
  }
  ...
  setAmbientLight(ambient) {
     ...
     */
    gl.useProgram(this.prog);
    gl.uniform1f(this.ambientPoint, ambient);
  }
}
...
const meshFS = `
          ...
          if(enableLighting)
          {
            vec3 texelVec3 = texture2D(tex, v_texCoord).rgb;
            vec3 normal = normalize(v_normal);
            float diffuse = dot(normal, -lightPos);
            diffuse = max(diffuse, 0.0);
  
            float sumI = ambientI + diffuse;
            vec3 sumIntensityVec3 = sumI * texelVec3;
            gl_FragColor = vec4(sumIntensityVec3, 1.0);
          }
          else{
